...
This style guide is based on [raywenderlich's swift style guide](https://github.com/raywenderlich/swift-style-guide).
The style guide's overarching goals are conciseness, readability, and simplicity.
...
Use `self` to access an object's properties or invoke its methods.
...
class BoardLocation {
  ...
  func printLocation() {
    println("Row: \(self.row), Column: \(self.column)")
  }
}
```
...
Try to shorten closures where the context is clear:
...
Credits to [raywenderlich](http:raywenderlich.com) and all [mentioned colaborators](https://github.com/raywenderlich/swift-style-guide).
